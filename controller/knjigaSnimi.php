...
if ($ISBN == "" || $Naziv == "" || $Autor == "" || $OznakaZanra == "") {
    die("Грешка: Сва обавезна поља морају бити попуњена.<br><br><a href=\"../unos.php\">ПОВРАТАК</a>");
}
...
if (strlen($Naziv) > 100) {
    die("Грешка: Назив књиге не сме бити дужи од 100 карактера.<br><br><a href=\"../unos.php\">ПОВРАТАК</a>");
}

// VALIDACIJA AUTORA
if (strlen($Autor) > 100) {
    die("Грешка: Аутор не сме бити дужи од 100 карактера.<br><br><a href=\"../unos.php\">ПОВРАТАК</a>");
}
...
if (!in_array($OznakaZanra, $dozvoljeniZanrovi)) {
    die("Грешка: Изабрани жанр није у дозвољеном домену вредности.<br><br><a href=\"../unos.php\">ПОВРАТАК</a>");
}
...
if (isset($_FILES["nazivFajlaSlike"]) && $_FILES["nazivFajlaSlike"]["error"] == 0) {
    ...
    if (!in_array($ekstenzija, $dozvoljeneEkstenzije)) {
        die("Грешка: Дозвољене су само JPG, JPEG и PNG слике.<br><br><a href=\"../unos.php\">ПОВРАТАК</a>");
    }
